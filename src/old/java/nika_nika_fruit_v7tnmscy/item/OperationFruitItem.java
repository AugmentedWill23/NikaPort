...
public class OperationFruitItem extends Item {
    public OperationFruitItem(Settings settings) { super(settings); }

    @Override
    public ActionResult use(World world, PlayerEntity user, Hand hand) {
        ItemStack stack = user.getStackInHand(hand);
        if (!world.isClient) {
            String fruitId = DevilFruitRegistry.FRUIT_OPERATION;
            ...
            if (user instanceof ServerPlayerEntity sp) {
                sp.sendMessage(Text.literal("§3You have consumed the Operation Fruit (Ope Ope no Mi)!"), false);
                sp.sendMessage(Text.literal("§7Controls: R cycle | Z use | X quick-use | O info | Passive Room + Waterwalk"), false);
            }
