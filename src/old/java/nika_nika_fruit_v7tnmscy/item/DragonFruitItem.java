...
public class DragonFruitItem extends Item {
    public DragonFruitItem(Settings settings) { super(settings); }

    @Override
    public ActionResult use(World world, PlayerEntity user, Hand hand) {
        ItemStack stack = user.getStackInHand(hand);
        if (!world.isClient) {
            String fruitId = DevilFruitRegistry.FRUIT_DRAGON;
            ...
            if (user instanceof ServerPlayerEntity sp) {
                sp.sendMessage(Text.literal("§bYou have consumed the Dragon-Dragon Fruit (Model: Seiryu)!"), false);
                sp.sendMessage(Text.literal("§7Controls: R cycle | Z use | X quick-use | O info | RShift: Fury Transform (Lv 250)"), false);
            }
